<div class="card-body">
    ...
    <div class="form-group row {{ $errors->has('max_use') ? 'has-error' : ''}}">
        <label for="max_use" class="col-sm-2 col-form-label">{{ 'Max usage per User' }}</label>
        <div class="col-sm-6">
            <input class="form-control" name="max_use" type="number" placeholder="Max use" id="max_use" value="{{ isset($coupon->max_use) ? $coupon->max_use : ''}}" >
        </div>
        <strong>{!! $errors->first('max_use', '<p class="help-block text-danger">:message</p>') !!}</strong>
    </div>
